...
chrome.runtime.onInstalled.addListener(async (details) => {
  ...
  if (details.reason === 'install') {
    const defaultMenuConfigs = {
      1: {
        title: 'Summary',
        provider: '',
        apiKey: '',
        prompt: 'Analyze the content of this link and provide a brief summary (one paragraph, no introduction, no structuring, no explanatory labels):',
        enabled: true
      },
      2: {
        title: 'Translation',
      ...
      3: {
        ...
        provider: '',
        apiKey: '',
        prompt: 'Analyze the content of this link and tell what can be found on it:',
        enabled: false
      }
    };
    
    await chrome.storage.sync.set({ menuConfigs: defaultMenuConfigs });
    console.log('Default menu configurations initialized');
  }
...
async function queryAI(url, config, pageData = null, selectedText = null) {
...
${selectedText}`;
    ...
    // If page content was successfully loaded
...
${pageData.content}`;
    ...
    fullPrompt = `${prompt || 'Analyze this link and tell what can be found on it:'} ${url}`;
